...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 28-04-2017 a las 17:54:39
...
('1302064942', 'Joel', 'Rivas', 'joel@gmail.com', '3984444444', '9836244444', 'Union Civica', 'e17ae9f0780022144ef4531b10ef14e018407386'),
...
(1, 14, 30, 5, 12, 15, 21, 11, 15, 6, 23, 10);
...
('PE-000010', 'TP-00012', '1302064942', '1234567890', '2017-04-28', '2017-04-28', '02:02:00', 1, 0, NULL),
...
CREATE TABLE `net_usuario` (
  ...
  `rol_usu` varchar(255) COLLATE utf8_spanish_ci DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_spanish_ci;
...
INSERT INTO `net_usuario` (`ced_usu`, `nom_usu`, `ape_usu`, `emi_usu`, `tel_usu`, `cel_usu`, `dir_usu`, `pass_usu`, `rol_usu`) VALUES
('1234567890', 'admin', 'admin', 'admin@admin.com', '1234567890', '1234567890', 'admin', 'd033e22ae348aeb5660fc2140aec35850c4da997', 'Administrador'),
('1302064942', 'Joel', 'Rivas', 'jarl.alejandro@gmail.com', '3887462222', '6777777777', 'alkshiudwer', 'cb44e1e1866061e0cbd1c8069e3dc794ba4dc12a', 'Tecnico'),
('1718760901', 'Alejandro', 'Rivas', 'jarlalejor@gmail.com', '3242342342', '2342342342', 'serfewer', '0472b31141ad5bafcb7d8c461fd3f063995612f5', 'Administrador');
...
CREATE VIEW `vista_trabajo`  AS  select `net_trabajo`.`tec_trab` AS `tec_trab`,`net_pedido`.`cod_ped` AS `cod_ped`,`net_pedido`.`clie_ped` AS `clie_ped`,`net_pedido`.`tecn_ped` AS `tecn_ped`,`net_pedido`.`fech_ped` AS `fech_ped`,`net_pedido`.`fevis_ped` AS `fevis_ped`,`net_pedido`.`hovis_ped` AS `hovis_ped`,`net_pedido`.`esta_ped` AS `esta_ped`,`net_pedido`.`es_factura` AS `es_factura`,concat(`net_cliente`.`nom_cli`,' ',`net_cliente`.`ape_cli`) AS `cliente`,`net_tipo`.`nom_tip` AS `nom_tip`,`net_pedido`.`serv_ped` AS `serv_ped`,concat(`net_usuario`.`nom_usu`,' ',`net_usuario`.`ape_usu`) AS `tecnico` from ((((`net_trabajo` join `net_pedido` on((`net_trabajo`.`cod_trab` = `net_pedido`.`cod_ped`))) join `net_cliente` on((`net_pedido`.`clie_ped` = `net_cliente`.`ced_cli`))) join `net_tipo` on((`net_tipo`.`cod_tip` = `net_pedido`.`serv_ped`))) join `net_usuario` on((`net_usuario`.`ced_usu` = `net_trabajo`.`tec_trab`))) ;
