...
Detalle.prototype = {
  ...
  build: function () {
    ...
    for(var i in db.materiales) {
      ...
      var template = `<tr>
        <td><input type="text" data-cant="${item.cantMax}" data-index="${i}" data-precio="${item.precio}" 
          class="form-control cant" placeholder="cantidad" onkeypress="numeros()" id="${i}_cant" 
          value="${item.cant}"/></td>
        <td>${item.nombre}</td>
        <td>${item.precio}</td>
        <td id="${i}_total">${item.total}</td>
        <td><input type="submit" class="btn btn-danger eliminar" value="Eliminar" data-index="${i}"/></td>
      </tr>`
    ...
    $(".cant").keyup(function (e) {
      ...
      if(parseInt(cant) < parseInt(input.val()) ){
        input.val(cant)
        ...
        db.materiales[index].cant = parseInt(input.val())
